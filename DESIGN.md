...
## Anti-Slop Directives: What to Strictly Avoid

Based on [Impeccable Design](https://impeccable.style/slop/), all future visual design changes in this project must avoid the following AI design habits:

### 1. Surfaces & Chrome
- **No Decorative Grids**: No subtle graph paper or grid lines behind cards or heroes.
- **No Glassmorphism Everywhere**: No frosted glass or glowing borders unless solving real layering problems.
- **No Hairline + Broad Shadow ("Ghost Cards")**: Never mix a crisp 1px border with a diffuse 30px shadow on the same card.
- **No Side-Tab Accent Borders**: No 3–4px colored stripes turning cards into fake alerts.
- **No Extreme Corner Radii**: No 32px–44px pill/blob cards that squash padding and content.
- **No Repeating Gradient Stripes**: No diagonal hazard lines or decorative noise fills.

### 2. Typography
- **No Kicker Labels Above Headings**: No small uppercase labels (`FEATURES`, `OVERVIEW`) stacked above headings.
- **No Hero Eyebrow Chips**: No floating pill badges (`INTRODUCING`) above headlines.
- **No Oversized Hero Headlines**: Keep hero titles purposeful without dominating the entire viewport.
- **No Flat Type Hierarchy**: Maintain distinct contrast in size, weight, and color across heading levels.
- **No Crushed Letter-Spacing**: No aggressive negative tracking that makes characters collide.
- **No Icon Tiles Stacked Above Headings**: No predictable rounded icon squares stacked over titles in card grids.
- **No Undersized Text**: All interactive and secondary interface text must be >= 13px.

### 3. Color & Contrast
- **No Cyberpunk / AI Gradient Reflex**: No saturated purple-to-blue gradients or neon accents.
- **No Radial Halos & Spotlights**: No glowing circular or elliptical gradient blobs behind text or surfaces.
- **No Gradient Text**: Never use linear gradient text clipping on headlines or metrics.
- **Strict WCAG AA**: Minimum 4.5:1 contrast for normal body text, 3:1 for large display text.

### 4. Layout & Spacing
- **No Nested Cards ("Cardocalypse")**: Flatten depth with whitespace and subtle hairlines rather than cards-inside-cards.
- **No Formulaic 3x3 Card Grids**: Vary layout according to content structure.
- **No Hero Metrics in Void**: No isolated giant numbers without meaningful explanatory context.
- **No Monotonous Spacing**: Group related items closely; separate distinct sections with generous space.
- **Respect Measure**: Cap reading line length to 60–75 characters (`max-w-5xl` for headings, `max-w-2xl` to `max-w-3xl` for body).
- **Responsive Gutters**: Always ensure `px-4 sm:px-8` side margins to avoid text touching screen edges.

### 5. Motion & Physics
- **No Pulsing Dots**: No animated pulse rings on static status indicators.
- **No Blinking Cursors**: No fake terminal blinking cursors on static copy.
- **Critically Damped Springs**: Use damping `1.0` or smooth `ease-out`; reserve bounce solely for momentum-driven touch gestures.
- **Hardware-Accelerated Motion**: Animate `transform` and `opacity` only; never animate `width`, `height`, or `margin`.

### 6. Microcopy
- **No Marketing Buzzwords**: Ban "supercharge", "enterprise-grade", "next-generation", "seamless", "world-class".
- **No Repeated Words**: Do not repeat the same label in the badge, heading, and button within one container.
- **No Em-Dash Overuse**: Avoid dashes in every sentence; use clean punctuation.
- **No Forced Contrast Slogans**: Avoid "Not a tool. A platform." style aphoristic pairs.
